...
def read_data(image_dir):
    ...
    filelist = glob(os.path.join(image_dir, '*/*_mask.jpg'))
    image_list = []
    label_list = []
    ...
    for file in filelist:
        ...
        image_list.append(cv2.imread(file.replace('_mask', ''), 1))
...
def main(image_dir, classifier, output_model):

    start = time.time()

    image_list, label_list = read_data(image_dir)
...
if __name__ == "__main__":
    ...
    main(image_dir, classifier, output_model)
